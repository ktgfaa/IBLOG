...
<link href="${contextPath }/resources/css/mypage/iblogfixForm.css?after" rel="stylesheet" type="text/css">
	...
	<div class="mypageForm_DIV">
		<div class="mypage_DISDIV">
			<div class="run_IBLOG_DIV">
				...
				<div>
					<form name="FIXIB" id="FIXIB" style="height: 800px;" action="${contextPath }/mypage/fixIblog.do" method="post">
						<table class="infoBlog_table">
						<tbody>
							<tr class="input_TR">
								<td><p>블로그 이름</p></td>
								<c:if test="${iblogInfoVO ne 'none' }">
									<td><input type="text" name="blogName" id="blogName" value="${iblogInfoVO.getBlogName() }"/></td>
								</c:if>
							...
							<tr class="input_TR">
								<td><p>서브 타이틀 제목</p></td>
								<c:if test="${iblogInfoVO ne 'none' }">
									<td><input type="text" name="title" id="title" value="${iblogInfoVO.getTitle() }"/></td>
								</c:if>
							...
							<tr class="input_TR">
								<td><p>블로그 주소</p></td>
								<c:if test="${iblogInfoVO ne 'none' }">
									<td><input type="text" name="blogAddress" id="blogAdress" value="${iblogInfoVO.getBlogAddress() }" readonly="readonly"/></td>
								</c:if>
							...
							<tr>
								<td><p>블로그 주제 선택</p></td>
								<td>
									<select name="subject" form="FIXIB">
			...
			<div class="run_IBLOG_DIV2">
				...
				<div class="categoryAdd_DIV">
					<form name="ADDCAT" id="ADDCAT" action="${contextPath }/mypage/addCategory.do" method="post">
						<table>
							<tbody id="categoryAdd_tbody">
								<c:if test="${categoryMainList == null }" >
									<tr class="main_category">
										<td class="cat_num_td">
											<input type="hidden" name="category_num" value="1" readonly="readonly"/>
											<input type="text" name="d" value="ㆍ" disabled="disabled" />
										</td>
										<td class="cat_main_td">
											<input type="text" name="category_value"/>
										</td>
										<td class="cat_btn_td">
											<input type="button" class="sub_add" value="추가" onClick="testFunction(event)"/>
											<input type="button" class="fix" value="수정"/><input type="button" class="del" style="margin:0;" value="삭제"/>
											<input type="hidden" name="category_kind" value="main" readonly="readonly"/>
										</td>
									</tr>
									...
									<tr class="sub_category">
										...
										<td class="sub_cat_main_td">
											<input type="hidden" name="category_num" class="sub_cnum" value="1-1" readonly="readonly"/>
											<input type="text" name="category_value" class="sub_cat"/>
										</td>
										<td class="sub_cat_btn_td">
											<input type="button" class="fix" value="수정"/>
											<input type="button" class="del" style="margin:0;" value="삭제"/>
											<input type="hidden" name="category_kind" value="sub" readonly="readonly"/>
										</td>
									</tr>
								...
								<c:if test="${categoryMainList != null }" >
									<c:forEach var="i" begin="0" end="${categoryMainList.size()-1}">
											<tr class="main_category">
												<td class="cat_num_td">
													<input type="hidden" name="category_num" value="${categoryMainList.get(i).get('CATEGORY_NUM') }" disabled="disabled"/>
													<input type="text" name="d" value="ㆍ" disabled="disabled" />
												</td>
												<td class="cat_main_td">
													<input type="text" name="category_value" value="${categoryMainList.get(i).get('CATEGORY_VALUE') }" disabled="disabled"/>
												</td>
												<td class="cat_btn_td">
													<input type="button" class="sub_add" value="추가" onClick="testFunction(event)"/>
													<input type="button" class="fix" value="수정" onClick="fixCatFunction(event)"/><input type="button" class="del" style="margin:0;" value="삭제" onClick="delCatFunction(event)"/>
													<input type="hidden" name="category_kind" value="main" disabled="disabled"/>
												</td>
											</tr>
										<c:forEach var="k" begin="0" end="${categorySubList.size()-1}">
												<c:if test="${categoryMainList.get(i).get('CATEGORY_NUM').charAt(0) == categorySubList.get(k).get('CATEGORY_NUM').charAt(0) }">
												<tr class="sub_margin_tr"></tr>
												<tr class="sub_category">
													<td class="sub_cat_num_td">
														<p>ㆍㆍㆍㆍ</p>
													...
													<td class="sub_cat_main_td">
														<input type="hidden" name="category_num" class="sub_cnum" value="${categorySubList.get(k).get('CATEGORY_NUM') }" disabled="disabled"/>
														<input type="text" name="category_value" class="sub_cat" value="${categorySubList.get(k).get('CATEGORY_VALUE') }" disabled="disabled"/>
													</td>
													<td class="sub_cat_btn_td">
														<input type="button" class="fix" value="수정" onClick="fixCatsubFunction(event)"/>
														<input type="button" class="del" style="margin:0;" value="삭제" onClick="delCatsubFunction(event)"/>
														<input type="hidden" name="category_kind" value="sub" disabled="disabled"/>
													</td>
												</tr>
												</c:if>
										</c:forEach>
						...
						<div class="save_div">
							...
							<input name="id" id="cat_id" type="hidden" value="${member.getUsername()}"/>
							<input type="button" id="main_add" value="메인 카테고리 추가"/>
							<input type="button" id="catego_save" value="변경사항 저장하기" />
						</div>
					</form>
				</div>
			</div>
...
function testFunction(event){
	...
	const num_first = main_category.children().first().children().attr("value");
	var sub_category = "";
	var total = 0;
	...
	if(classsub == null) {
		...
		$("<tr class='sub_category'>" +
			"<td class='sub_cat_num_td'><p>ㆍㆍㆍㆍ</p></td>" +
			"<td class='sub_cat_main_td'><input type='hidden' name='category_num' class='sub_cnum' value='"+num_first+"-"+(total+1)+"' readonly='readonly'/><input type='text' name='category_value' class='sub_cat' placeholder='소분류는 최대 16개까지 만들수 있습니다'/></td>" +
			"<td class='sub_cat_btn_td'><input type='button' class='fix' value='수정'/><input type='button' class='del' style='margin:0;' value='삭제' onClick='delCatsubFunction(event)'/><input type='hidden' name='category_kind' value='sub' readonly='readonly'/></td>" +
	...
	function delCatFunction(event) {
		...
		for(var i=0;i<16;i++) {
			...
			if(main_cat2.attr("class") == 'sub_category' || main_cat2.attr("class") == 'main_margin_tr' || main_cat2.attr("class") == 'sub_margin_tr' ) {
				list.push(main_cat2);
			} else if(main_cat2.attr("class") == 'main_category') {
				console.log(list);
				category_num.push(main_cat.children().eq(0).children().eq(0).attr("value"));
				for(var k=0;k<list.length;k++){
					if(k%2 != 0 && k != 0) {
						category_num.push(list[k].children().eq(1).children().eq(0).attr("value"));
					}
					list[k].remove();
				}
