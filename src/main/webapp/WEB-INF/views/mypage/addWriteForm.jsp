...
<c:choose>
	<c:when test="${write_main_category eq 'none' || write_sub_category eq 'none' }">
		<script>
			window.onload=function() {
...
<link href="${contextPath }/resources/css/mypage/addWriteForm.css?after" rel="stylesheet" type="text/css">
...
<%-- <script type="text/javascript" src="${contextPath }/ckeditor/ckeditor.js" charset="utf-8"></script> --%>
<link
	...
	<div class="mypageForm_DIV">
		<div class="mypage_DISDIV">
			<div class="run_IBLOG_DIV">
				...
				<div>
					<form name="WRITEIB" id="WRITEIB" style="min-height: 1300px;" action="${contextPath }/mypage/addWrite.do" method="post">
						<ul>
							<li>
								<select name="write_main_category" id="main" form="WRITEIB" onchange="main_categoryValue()">
									...
									<c:if test="${write_main_category ne 'none' && write_sub_category ne 'none' }">
										<c:forEach var="i" begin="0" end="${write_main_category.size()-1}">
											<c:if test="${main_value ne 'none' && main_value eq write_main_category.get(i).get('CATEGORY_VALUE') }">
												<option value="${write_main_category.get(i).get('CATEGORY_VALUE') }" selected="selected">${write_main_category.get(i).get('CATEGORY_VALUE') }</option>
											</c:if>
											<c:if test="${main_value ne 'none' && main_value eq write_main_category.get(i).get('CATEGORY_VALUE') }">
												<option value="${write_main_category.get(i).get('CATEGORY_VALUE') }">${write_main_category.get(i).get('CATEGORY_VALUE') }</option>
											</c:if>
							...
							<li>
								<input type="text" name="write_subject" id="write_title" placeholder="글제목"/>
							</li>
							<li>
 								<div class="jsx-2303464893 editor"> 
									<div class="fr-box fr-basic fr-top" role="application"> 
										<div class="fr-wrapper show-placeholder" style="overflow: scroll;width: 1148px;"> 
											<textarea class="form_control" name="write_description" id="bo_content" style="width: 1124px; height: 500px;">
											</textarea> 
										</div> 
									</div> 
								</div>
<!-- 								<div style = "width:1124px;">
							...
							<li>
								...
								<input name="id" id="write_id" type="hidden" value="${member.getUsername()}"/>
							</li>
							<li>
								<input type="button" id="saveWrite" value="완료" />
							</li>
						</ul>
					</form>
				</div>
