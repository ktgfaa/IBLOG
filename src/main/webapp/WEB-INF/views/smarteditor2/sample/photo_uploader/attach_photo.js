	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src","/iblog/smarteditor2/sample/photo_uploader/img/btn_confirm2.png");
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
	    	 ...
	    	 welBtnConfirm.attr("src","/iblog/smarteditor2/sample/photo_uploader/img/btn_confirm.png");
    	 }
     }   
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
	        ...
	        aFileList.push('	<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button" src="/iblog/smarteditor2/sample/photo_uploader/img/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
			aFileList.push('	</li> ');   
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/iblog/smarteditor2/file_uploader_html5.do'; 	//upload URL
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						console.log("res._response.responseText : " + res._response.responseText);
    ...
    function makeArrayFromString(sResString){
 		...
 		try{
	    	...
	    	for (var i = 0; i < aTemp.length ; i++){
	    		if( !!aTemp[i] && aTemp[i] != "" && aTemp[i].indexOf("=") > 0){
	    			...
	    			console.log("aSubTemp : " + aSubTemp[1]);
	    			htTemp[aSubTemp[0]] = aSubTemp[1];
	    			htTemp[aSubTemp[0]] = htTemp[aSubTemp[0]].split("</div>");
	    			htTemp[aSubTemp[0]] = htTemp[aSubTemp[0]][0];
	    			console.log("htTempS : " + htTemp[aSubTemp[0]]);
	    		}
	 		}
 		}catch(e){}
    	...
    	if(aResult.length == nImageFileCount){
    		console.log("aResult[aResultleng] : " + aResult[aResultleng]);
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/smarteditor2/singleImageUploader.do',	//샘플 URL입니다.
 	        sCallback : '/iblog/smarteditor2/sample/photo_uploader/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
